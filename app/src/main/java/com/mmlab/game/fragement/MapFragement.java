...
public class MapFragement extends Fragment
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        long sec  = GameActivity.getGame_data().getEnd_time();
        millsec = sec*1000L;


        Log.d("OAO", "ID : " + GameActivity.getGame_data().getId() );
        Log.d("OAO", "start_time : " + GameActivity.getGame_data().getStart_time() );
        Log.d("OAO", "end_time1 : " + GameActivity.getGame_data().getEnd_time() );
        Log.d("OAO", "end_time2 : " + millsec );
        Log.d("OAO", "play_time : " + GameActivity.getGame_data().getPlay_time() );
        Log.d("OAO", "is_playing : " + GameActivity.getGame_data().getGroup_id_id() );
        ...
        if(millsec >= 0) {
            countDownTimer = new CountDownTimer(millsec, 1000) {
                ...
                public void onTick(long millisUntilFinished) {
                    ...
                    if(day != 0){
                        countdown.setText("剩餘 " + day + "天" + hour + ":" + min + ":" + sec);
                    }
                    else if(hour != 0){
                        countdown.setText("剩餘 " + hour + ":" + min + ":" + sec);
                    }else{
...
//                showVentureEndingDialog(getActivity(), marker);
        ...
        minusCall.enqueue(new Callback<String>() {
            ...
            public void onResponse(Call<String> call, Response<String> response) {
                ...
                if(response.body().equals("chest is empty")){
                    ...
                    Toast.makeText(getActivity(), "OMGGGGGGGGGGGGG", Toast.LENGTH_LONG).show();
                }
            }
    ...
    public void apiAnswerRecord(final AnswerRecord answerRecord,final int chestPoint){
        ...
        call.enqueue(new Callback<String>() {
            @Override
            public void onResponse(Call<String> call, Response<String> response) {
                Log.d("msg", " update the answer :" + response.body());



                if (response.body().equals("already answer") || response.body() == "already answer"){
                    Toast.makeText(getActivity(), "上傳失敗，您已經回答過了....", Toast.LENGTH_LONG).show();
                    closeDialog();
                    forceUpdateMarker();
                }
                else if(response.body().equals("finish") || response.body() == "finish"){
